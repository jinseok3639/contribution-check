...
(위 clone은 기본 브랜치인 `master` — 배포용으로 스킬 파일만 펼쳐둔 레이아웃 — 를 받는다. 지금 보고 있는 `dev`는 개발용이라 구조가 다르다. 아래 "개발" 참고.)
...
- **AI 사용 여부는 판별하지 않는다.** `Co-Authored-By` 트레일러는 꺼두는 경우가 흔하고 AI 코딩 도구가 사람 이름처럼 보이는 트레일러를 남기기도 해서 신뢰할 수 없다. git author 자체를 귀속 기준으로 삼는다.
...
- **`master`** (기본 브랜치, 배포용) — 스킬 파일만 루트에 펼쳐둬서 `~/.claude/skills/contribution-check`로 바로 clone할 수 있다.
- **`dev`** (이 브랜치, 개발용) — 스킬 원본이 `.claude/skills/contribution-check/`에 있어 이 레포 세션에서 바로 실행·시험할 수 있다. 설계 노트(`note/`), 시험 분석 결과(`sample_result/`), 시험 대상 레포(`sample_repo/`)는 개인 작업 자료라 커밋하지 않는다(`.gitignore`) — GitHub에서는 보이지 않고 로컬 작업 사본에만 있다.

두 브랜치는 파일 경로가 달라 merge로 잇지 않는다. dev에서 고친 내용을 master로 내보낼 때는:
...
스킬 폴더 내용물과 `release/`(배포 전용 README·LICENSE·gitignore)를 합쳐 master 트리를 통째로 다시 만든다. 별도 worktree에서 작업하므로 지금 작업본은 건드리지 않는다.

## 구성

```
.claude/skills/contribution-check/
...
└── references/
    ├── report-template.html          리포트 스타일·스크립트 원본
    └── analysis-example.json         analysis.json 스키마 예시
scripts/release.py                    dev -> master 배포 스크립트
release/                              master 전용 README·LICENSE·gitignore
note/                                 설계 노트 (개인 작업 자료, 커밋하지 않음)
```
